...
function Storage (store, ref) {
  ...
  var api = function (key, value, clear) {
    var storeKey = namespace + key;
    ...
    if (key && clear === true) {
      ...
      if (api[key]) {
        ...
        api[key].map(function (v) { return store.setItem(storeKey, serialize(v)); });
        return api[key]
      }
    } else {
  ...
  Object.keys(store)
    ...
    .forEach(function (k) { return api[ k.slice(namespace.length) ] = stream(deserialize(store.getItem(k))); });
